...
control.addTo(map);
control.hide();
controlHTML = document.getElementsByClassName("leaflet-routing-geocoders")[0];
/* ================================*/
    ...
    pointToLayer: function(geoJsonPoint, latlng) {
      ...
      marker.on('click', function(event){
        ...
        L.DomEvent.on(startBtn, 'click', function() {
            control.show();
              controlsHTML.forEach(item => item.classList.remove('disabled'));
...
searchControlHTML = document.getElementsByClassName("leaflet-control-search")[0];
...
lc.addTo(map);
lcHTML = document.getElementsByClassName("leaflet-control-locate")[0];
/* ================================*/

var controlsHTML = [searchControlHTML, lcHTML];
...
showGeocoderBtn.onclick = function () {
  if (itineraryShown) {
    clearWaypoints();
    control.hide()
    // Скроет кнопку, когда доведём до ума
    // showGeocoderBtnHTML.classList.remove('disabled');
    controlsHTML.forEach(item => item.classList.add('disabled'));
    itineraryShown = !itineraryShown;
  } else {
    control.show();
    // Отобразит кнопку
    // showGeocoderBtnHTML.classList.add('disabled');
    controlsHTML.forEach(item => item.classList.remove('disabled'));
    itineraryShown = !itineraryShown;
  }
};
